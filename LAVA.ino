...
#define VISUALS   3   //Number of visuals in the switch case

#define AUDIO_PIN A0  //Pin for the envelope (DO) of the sound detector
...
uint16_t gradient = 0; //Used to iterate and loop through each color palette slowly over time
...
//  keep color gradient from overflowing, the color functions themselves can take any positive value. For example, the
//  largest value Rainbow() takes before looping is 1529, so "gradient" should reset after 1529.
//     Make sure you add/remove values accordingly if you add/remove a color function in the switch-case in colorPalette().
uint16_t thresholds[] = {1529, 1019, 764};
...
float maxVol = 15;    //Holds the largest volume recorded thus far to adjust the visual's responsiveness.
float knob = 1023.0;  //Holds the percentage of how twisted the trimpot is. Used for adjusting the max brightness.
float avgBump = 0;    //Holds the "average" volume-change to trigger a "bump" or a change in the lighting effect.
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//      The reason average is quoted is because its not actually the true mathetmical average. I used what I 
//      call a sequenced avg. in which the only true difference is that it doesn't take its values from the pool
//      of all previously recorded values, but averages the last average and the current value received.
//      I'm really bad at explaining things so here's the math
//
//          True avg: (1 + 2 + 3) / 3 = 2
//          Sequenced avg: (1 + 2) / 2 = 1.5 --> (1.5 + 3) / 2 = 2.25  (if 1, 2, 3 was the order the values were received)
//
//      All "averages" in the program operate this way.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void loop() {  //This is where the magic happens. This loop produces each frame of the visual.
  ...
  delay(15);     //Paces visuals so they aren't too fast to be enjoyable
}
//////////</Standard Functions>
...
//      maxVol, so that louder = brighter.
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CycleVisual() {
  ...
  //  Thanks to Jessie for helping me figure this out...would never have gotten it.
  if (shuffle && millis() / 1000.0 - shuffleTime > 30 && !(gradient % 2)) {
...
//  Gradients should (TM) also loop back to their starting color so there's no jumps in color.

uint32_t Rainbow(unsigned int i) {
  if (i > 1529) return Rainbow(i % 1530);
  if (i > 1274) return strand.Color(255, 0, 255 - (i % 255));   //violet to red
  if (i > 1019) return strand.Color((i % 255), 0, 255);         //blue to violet
  if (i > 764) return strand.Color(0, 255 - (i % 255), 255);    //aqua to blue
  if (i > 509) return strand.Color(0, 255, (i % 255));          //green to aqua
  if (i > 255) return strand.Color(255 - (i % 255), 255, 0);    //yellow to green
  return strand.Color(255, i, 0);                               //red to yellow
}

uint32_t Sunset(unsigned int i) {
  if (i > 1019) return Sunset(i % 1020);
  if (i > 764) return strand.Color((i % 255), 0, 255 - (i % 255));          //blue to red
  if (i > 509) return strand.Color(255 - (i % 255), 0, 255);                //purple to blue
  if (i > 255) return strand.Color(255, 128 - (i % 255) / 2, (i % 255));    //orange to purple
  return strand.Color(255, i / 2, 0);                                       //red to orange
}

uint32_t Ocean(unsigned int i) {
  if (i > 764) return Ocean(i % 765);
  if (i > 509) return strand.Color(0, i % 255, 255 - (i % 255));  //blue to green
  if (i > 255) return strand.Color(0, 255 - (i % 255), 255);      //aqua to blue
  return strand.Color(0, 255, i);                                 //green to aqua
}

uint32_t PinaColada(unsigned int i) {
  if (i > 764) return PinaColada(i % 765);
  if (i > 509) return strand.Color(255 - (i % 255) / 2, (i % 255) / 2, (i % 255) / 2);  //red to half white
  if (i > 255) return strand.Color(255, 255 - (i % 255), 0);                            //yellow to red
  return strand.Color(128 + (i / 2), 128 + (i / 2), 128 - i / 2);                       //half white to yellow
}

uint32_t Sulfur(unsigned int i) {
  if (i > 764) return Sulfur(i % 765);
  if (i > 509) return strand.Color(i % 255, 255, 255 - (i % 255));   //aqua to yellow
  if (i > 255) return strand.Color(0, 255, i % 255);                 //green to aqua
  return strand.Color(255 - i, 255, 0);                              //yellow to green
}

uint32_t NoGreen(unsigned int i) {
  if (i > 1274) return NoGreen(i % 1275);
  if (i > 1019) return strand.Color(255, 0, 255 - (i % 255));         //violet to red
  if (i > 764) return strand.Color((i % 255), 0, 255);                //blue to violet
  if (i > 509) return strand.Color(0, 255 - (i % 255), 255);          //aqua to blue
  if (i > 255) return strand.Color(255 - (i % 255), 255, i % 255);    //yellow to aqua
  return strand.Color(255, i, 0);                                     //red to yellow
}
